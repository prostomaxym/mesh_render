...
MeshA::MeshA(std::string fname)
{
	...
	while (!fin.eof()) {
		...
		if (line.find("v ") == 0) {
			...
			const vec3f v1(&vert_palette[(a - 1) * 3]);
...
void MeshA::load(std::string fname)
{
	//Open the file for reading
		std::ifstream fin(fname);
	if (!fin) {
...
void MeshA::addFace(const std::vector<float>& vert_palette, const std::vector<float>& uv_palette,
	uint32_t a, uint32_t at, uint32_t b, uint32_t bt, uint32_t c, uint32_t ct, bool is3DTex)
{
	...
	vec3f v1(&vert_palette[a * 3]);
	vec3f v2(&vert_palette[b * 3]);
	vec3f v3(&vert_palette[c * 3]);
	vec3f dv1 = v2 - v1;
	vec3f dv2 = v3 - v1;
	vec3f normal = dv1 ^ dv2;
	normal.normalise();
